          ...
          if ~isfield(gp.lik, 'nondiagW')
            ...
            switch gp.lik.type
              ...
              case {'Softmax', 'Multinom'}
                ...
                % initialize latent values. Zero seems to be a robust choice (Jarno)
                % with mean functions, initialize to mean function values
                if ~isfield(gp,'meanf')
                    ...
                    elseif isequal(gp.lik.type, 'inputdependentt')
                        f = zeros(sum(nl), 1);
                        f(1:n) = 0;
                        f(n + 1:2*n) = 3;
                        
                    else
                        % no fancy likelihoods
                        f = zeros(sum(nl), 1);
                    end
                else
                    [H, b_m, B_m] = mean_prep(gp, x, []);
                ...
                % K is block-diagonal covariance matrix where blocks correspond to latent processes
                K = zeros(sum(nl));
                if isfield(gp.lik, 'xtime')
                ...
                % cholesky decomposition of block diagonal K
                % cL = chol(K);
                ...
                % –––––––––––––––––––––––––––––––––––––––––––––––––––––––––––
                % NEWTON-ALGORITHM, see Rasmussen & Williams (2006) | page 46
                ...
                if isfield(gp.lik, 'xtime')
                    ...
                    % indexes for the diagonal matrix
                    ind1 = repelem((1 : (nlp + 1) : nlp^2) * n + 1 - n, n) + repmat((0:(n - 1)), 1, nlp);

                    % auxiliar indexes for non-diagonal and diagonal elements
                    ind2 = reshape(1 : n * nlp, n, nlp)';
                    
                    %if isequal(gp.lik.type, 'inputdependentt') 
                    if isequal(gp.lik.type, 'inputdependentt') && strcmp(gp.lik.fisher, 'on')
                        % in this case we use the expected Fisher information matrix in place of W, W = -E[d²ll/df²]
                        Wvec = gp.lik.fh.fi(gp.lik, y, f, 'latent', z);
                        Wdiag = Wvec(ind1)';
                ...
                % dlp = d(log(p(y|f)))/df | gradients of the log-likelihood w.r.t. latent process
                dlp = gp.lik.fh.llg(gp.lik, y, f, 'latent', z);
                
                % lp_new = log(p(y|f)) | log-likelihood
                lp_new = gp.lik.fh.ll(gp.lik, y, f, z);
                lp_old = - Inf;
                WK = zeros(sum(nl));
                iter = 0;
                ...
                while abs(lp_new - lp_old) > tol && iter < maxiter 
                    ...
                    if isfield(gp.lik, 'xtime')
                        ...
                        for i1 = 1:nlp
                            for j1 = 1:nlp
                                WK(ind2(i1, :), ind2(j1, :)) = bsxfun(@times, Wvec(ind2(i1, :), j1), K(ind2(j1, :), ind2(j1, :)));
                            end
                        end
                    end

                    B = WK;
                    B(1 : n*nlp + 1 : end) = B(1 : n*nlp + 1 : end) + 1;
                    [ll, uu] = lu(B);
                    
                    % a = inv(I+WK)*(W*f - d(log(p(y|f)))/df) | a = B\b;
                    a = uu \ (ll \ b);
                    f = K*a;
                    ...
                    if ~isfield(gp,'meanf')
                        ...
                        % lp_new = -(f-H'*b_m)'*(a-K\(H'*b_m))/2 + lp | f^=f-H'*b_m | f^=f-Hb_m,
                        lp_new = -(f-Hb_m)'*(a-iKHb_m)/2 + lp;
                    end
                    ...
                    % step-size reduction
                    i = 0;
                    %while i < 20 && lp_new < lp_old && ~isnan(sum(f))
                    while i < 25 && lp_new < lp_old && ~isnan(sum(f))
                        % reduce step size by halfpause
                        a = (a_old + a)/2;    
                        ...
                        %clf,plot(f(1:length(y)), 'b'),hold on,plot(f(length(y)+1:2*n), 'r'),plot(f(2*length(y)+1:end),'g'),plot(y,'.'),pause
                        %clf,plot(f(1:length(y))),hold on,plot(f(length(y)+1:end)),plot(y,'.'),pause
                        %clf, plot(x(:, 1), f(1:length(y)),  'b'), hold on, plot(x(:, 1), f(length(y)+1:2*n), 'r'), plot(x(:, 1), f(2*length(y)+1:3*n), 'g'), plot(x(:, 1), y,'.'), pause
                    end
                    % –––––––––––––––––––––––––––––––––––––––––––––––––––––
                    ...
                    % iter
                    % clf;
                    % plot(x(:, 1), f(1:length(y)),  'b'),hold on,plot(x(:, 1), f(length(y)+1:2*n), 'r'),plot(x(:, 1), y,'.');
                end
                
                %if iter == maxiter
                %    display(iter); display(lp_new);
                %end
                
                % clf,plot(f(1:length(y)), 'b'),hold on,plot(f(length(y)+1:2*n), 'r'),plot(f(2*length(y)+1:end),'g'),plot(y,'.')
                % plot(f(1:length(y)), 'b'),hold on,plot(f(length(y)+1:2*n), 'r'),plot(f(2*length(y)+1:end),'g'),plot(y,'.')
                % plot(x(:, 1), f(1:length(y)),  'b'),hold on,plot(x(:, 1), f(length(y)+1:2*n), 'r'),plot(x(:, 1), y,'.')
                % plot(x(:, 1), f(1:length(y)),  'b'), hold on,
                % plot(x(:, 1), f(length(y)+1:2*n), 'r'), hold on,
                % plot(x(:, 1), f(2*length(y)+1:3*n), 'g'), plot(x(:, 1), y,'.')
                ...
                elseif isequal(gp.lik.type, 'inputdependentt') && strcmp(gp.lik.fisher, 'on')
                    Wvec = gp.lik.fh.fi(gp.lik, y, f, 'latent', z);
                    Wdiag = Wvec(ind1)';
                ...
                if isequal(gp.lik.type, 'inputdependentt')  
                    ...
                    % Take block matrices
                    B1  = B(ind2(1, :), ind2(1, :));  B12 = B(ind2(1, :), ind2(2, :));
                    B21 = B(ind2(2, :), ind2(1, :));  B2  = B(ind2(2, :), ind2(2, :)); 
                    
                    % LU decomposition of B2 (seems to be more stable);
                    [L2, U2] = lu(B2);
                    
                    % inverse B2 times B21
                    B2invB21 = U2 \ (L2 \ B21);
                    
                    % determinants
                    logdet1 = det(L2) * prod(sign(diag(U2))) .* sum(log(abs(diag(U2))));
                    
                    BB = B1 - B12 * B2invB21;
                    [L3, U3] = lu(BB);
                    logdet2 = det(L3) * prod(sign(diag(U3))) .* sum(log(abs(diag(U3))));
                    
                    % log marginal likelihood 
                    edata = logZ + 0.5 * (logdet1 + logdet2);

                    % if any(isnan(B)) | any(isinf(B))
                    %    warning('Nan or inf →→→→→ B');
                    % end
                ...
                % Return help parameters for gradient and prediction, calculations
                L = B;
                E = Ll;
                M = Lu;
            end
