          ...
          case {'Softmax', 'Multinom'}
            ...
            otherwise
              ...
              if isfield(gp.lik, 'xtime')
                  ...
                  % indexes for diagonal matrix
                  ind1 = repelem((1:(nlp+1):nlp^2)*tn + 1 - tn, tn) + repmat((0:(tn-1)), 1, nlp);
                  
                  % auxiliar indexes for the training points
                  nlind = reshape(1:tn*nlp, tn, nlp)';    
                  
                  % auxiliar indexes for the test cases
                  nltind = reshape(1:ntest*nlp, ntest, nlp)';
                  
                  % second derivatives of log-likelihood
                  Wvec = -gp.lik.fh.llg2(gp.lik, y, f, 'latent', z);
                  
                  if isequal(gp.lik.type, 'inputdependentt') && strcmp(gp.lik.fisher, 'on')
                      [~, Wvec2] = gp.lik.fh.fi(gp.lik, y, f, 'latent', z);
                  elseif isequal(gp.lik.type, 'inputdependentt')
                      [~, Wvec2] = gp.lik.fh.llg2(gp.lik, y, f, 'latent', z);
                      Wvec2 = -Wvec2;
                  end
              ...
              % K_nf is K(x, xt) covariance matrix where blocks correspond to latent processes
              K_nf = zeros(sum(nlt), sum(nl));
              if isempty(predcf)
                  K_nf(nltind(2, :), nlind(2, :)) = gp_cov(gp, xt, x, gp.comp_cf{2});
                  
                  if isfield(gp.lik, 'xtime')
                      K_nf(nltind(1, :), nlind(1, :)) = gp_cov(gp,xtime, xtime, gp.comp_cf{1});
                  else
                      K_nf(nltind(1, :), nlind(1, :)) = gp_cov(gp,xt,x, gp.comp_cf{1});
                  end
              else
                  K_nf(nltind(2, :), nlind(2, :)) = gp_cov(gp, xt, x, intersect(gp.comp_cf{2}, predcf));
                  if isfield(gp.lik, 'xtime')
                      K_nf(nltind(1, :), nlind(1, :)) = gp_cov(gp,xtime, xtime, intersect(gp.comp_cf{1}, predcf)); 
                  else
                      K_nf(nltind(1, :), nlind(1, :)) = gp_cov(gp,xt,x, intersect(gp.comp_cf{1}, predcf));
                  end
              end
              ...
              if nargout > 1
                  ...
                  % Evaluate the variance
                  % Kss is K(X*, X*) covariance matrix between test points, where each block corresponds to latent processes
                  Kss = zeros(sum(nlt));
                  
                  if isempty(predcf)
                      Kss(nltind(2, :), nltind(2, :)) = gp_trcov(gp,xt,gp.comp_cf{2});
                    
                      if isfield(gp.lik,'xtime')
                          Kss(nltind(1, :), nltind(1, :)) = gp_trcov(gp,xtime,gp.comp_cf{1});
                      else
                          Kss(nltind(1, :), nltind(1, :)) = gp_trcov(gp,xt,gp.comp_cf{1});
                      end
                      
                  else
                      Kss(nltind(2, :), nltind(2, :)) = gp_trcov(gp,xt,intersect(gp.comp_cf{2}, predcf));
                      if isfield(gp.lik,'xtime')
                          Kss(nltind(1, :), nltind(1, :)) = gp_trcov(gp,xtime,intersect(gp.comp_cf{1}, predcf));
                      else
                          Kss(nltind(1, :), nltind(1, :)) = gp_trcov(gp,xt,intersect(gp.comp_cf{1}, predcf));
                      end
                  end
                  ...
                  if isfield(gp.lik,'xtime')
                     ...
                     % iBW = U1 \ (L1 \ Wvec2);
                      iBW = U1 \ (L1 \ sparse(Wvec2));
                     %  iBW = zeros(sum(tn * nlp));
                     %  for ii = 1:nlp
                     %      for jj = ii:nlp
                     %          for kk = 1:nlp
                     %              iBW(nlind(ii, :), nlind(jj, :)) = iBW(nlind(ii, :), nlind(jj, :)) ...
                     %                  + bsxfun(@times, iB(nlind(jj, :), nlind(kk, :)), Wvec(nlind(kk, :), jj));
                     %          end
                     %          if ii ~= jj
                     %              iBW(nlind(jj, :), nlind(ii, :)) = iBW(nlind(ii, :), nlind(jj, :))';
                     %          end
                     %      end
                     %  end
                  end
