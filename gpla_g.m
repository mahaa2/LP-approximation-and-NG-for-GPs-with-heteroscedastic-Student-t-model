...
%             end
          ...
          otherwise
              ...
              if isfield(gp.lik, 'xtime')
               ...
               % input-dependent
                  ...
                  if isequal(gp.lik.type, 'inputdependentt') && strcmp(gp.lik.fisher, 'on')
                      ...
                      Wvec2 = sparse(Wvec2);
                      Wvec3 = -gp.lik.fh.llg2(gp.lik, y, f, 'latent', z);
                      Wdiag = Wvec(ind1)';
              ...
              if isfield(gp.lik, 'xtime')
                  ...
                  for i1 = 1:nlp
                      for j1 = 1:nlp
                          KW(ind2(i1, :), ind2(j1, :)) = bsxfun(@times, K(ind2(i1, :), ind2(i1, :)), Wvec(ind2(i1, :), j1)');
                          
                          if isequal(gp.lik.type, 'inputdependentt') && strcmp(gp.lik.fisher, 'on')
                              KW2(ind2(i1, :), ind2(j1, :)) = bsxfun(@times, K(ind2(i1, :), ind2(i1, :)), Wvec3(ind2(i1, :), j1)');
                          end
                      end
                  end
              end
              ...
              if isequal(gp.lik.type, 'inputdependentt') && strcmp(gp.lik.fisher, 'on')
                  B2 = KW2; 
                  B2(1 : n*nlp + 1: end) = B2(1 : n*nlp + 1 : end) + 1; 
                  [L2, U2] = lu(B2);
              end
              ...
              if isfield(gp.lik, 'xtime')
                  ...
                  % third derivatives
                  % [dw_mat, dWdf1, dWdf2, dWdf3] = gp.lik.fh.llg3(gp.lik, y, f, 'latent', z);
                  if isequal(gp.lik.type, 'inputdependentt') && strcmp(gp.lik.fisher, 'on')
                      dw_mat = -gp.lik.fh.dfi(gp.lik, y, f, 'latent', z);
                  else
                      dw_mat = gp.lik.fh.llg3(gp.lik, y, f, 'latent', z);
                  end

                  for i1 = 1:n
                      ind3 = i1:n:(i1+(nlp - 1)*n);
                      for j1 = 0 : nlp - 1
                          % s2(i) = -0.5*trace(inv(inv(K)+W)*dW/df_i)
                          s2(i1 + j1*n) = 0.5 * trace(A(ind3, ind3) * dw_mat(:, :, j1 + 1, i1));
                      end
                  end
              ...
              if ~isempty(strfind(gp.infer_params, 'covariance'))
                  ...
                  for i=1:ncf
                      ...
                      if isfield(gp.lik, 'xtime')
                          ...
                          %      for jj = ii:nlp
                          %          for kk = 1:nlp
                          %              WiB(ind2(ii, :), ind2(jj, :)) = WiB(ind2(ii, :), ind2(jj, :)) ...
                          %                  + bsxfun(@times, Wvec(ind2(ii, :), kk), iB(ind2(kk, :), ind2(jj, :)));
                          %          end
                          %          if ii ~= jj
                          %              WiB(ind2(jj, :), ind2(ii, :)) = WiB(ind2(ii, :), ind2(jj, :))';
                          %         end
                          %      end
                          %  end
                      end
              ...
              if ~isempty(strfind(gp.infer_params, 'likelihood')) && ~isempty(gp.lik.fh.pak(gp.lik))
                  ...
                  if ~isempty(g_logPrior)
                      ...
                      if isequal(gp.lik.type, 'inputdependentt') && strcmp(gp.lik.fisher, 'on')
                          DW_thlik = -lik.fh.dfi(lik, y, f, 'latent2+param', z);
                      else
                          DW_thlik = lik.fh.llg3(lik, y, f, 'latent2+param', z);
                      end
                      
                      % gradient w.r.t. function values 
                      DL_thlik = lik.fh.llg(lik, y, f, 'param', z);
                      
                      % derivative of the self-consistent equation here
                      b = K * lik.fh.llg2(lik, y, f, 'latent+param', z);
                      ...
                      % sum(sum((A.*DW_sigma)))
                      for ii = 1:size(DL_thlik, 2)
                          DW = DW_thlik((1:n*nlp), (1:nlp*n) + (ii-1)*n*nlp);
                          ind4 = (DW ~= 0);
                          ssADW(ii) = sum(A(ind4) .* DW(ind4));
                          % ssADW(ii) = sum(A2(ind4) .* DW(ind4));
                      end
                      
                      gdata_lik = - DL_thlik - 0.5.*ssADW - s2'*s3;
